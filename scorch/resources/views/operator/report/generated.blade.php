<x-app-layout>
    ...
    <div class="container flex-inline bg-white" id="printableArea">
        ...
        <div id="weekContainer" class="item pt-5 pb-5">
            <h2 class="text-lg font-medium">Resumen de Disponibilidad</h2>
            <p class="text-sm">A continuación la información de disponibilidad resumida por semanas</p>
            @foreach ($weekRecs->groupBy('weekNum') as $week)
            <div id="week" class="py-3 ">
                <h2 class="text-md font-medium">Semana {{($week[0]->weekNum)}}</h2>
                <br>
                <div class="flex text-sm">
                    <div class="item w-32 text-gray-400">
                        <div class="text-transparent">Fecha</div>
                        <div class="text-transparent">Hora</div>
                        <div class="border-t border-b h-34 py-1">Activos</div>
                        ...
                        <div class="border-b h-34 py-1">Total Entregados</div>
                        <div class="border-b h-34 py-1">Total Antamina</div>
                        <div class="border-b h-34 py-1">Total SSEE</div>

                    </div>
                    @foreach ($week as $r)
                    <div id="day" class="item w-32 text-center" style="width: 10ch">
                        <div id="dayDate" class="h-34 text-gray-400">
                            {{\Carbon\Carbon::createFromFormat('Y-m-d H:i:s', $r->date)->format('d/m/Y h:i a')}}</div>
                        <div class="border-b border-t h-34 py-1">{{$r->cTotal}}</div>
                        <div class="border-b h-34 py-1 ">{{$r->cAntamina}}</div>
                        <div class="border-b h-34 py-1 ">{{$r->cSsee}}</div>
                        <div id="dayConns" class="border-b h-34 py-1 ">{{$r->pTotal}}</div>
                        <div class="border-b h-34 py-1 ">{{$r->dTotal}}</div>
                        <div class="border-b h-34 py-1 ">{{$r->dAntamina}}</div>
                        <div class="border-b h-34 py-1 ">{{$r->dSsee}}</div>
                    </div>
                    @endforeach
                </div>
                <br>
                <div id="wChart" class="w-full"></div>
            </div>
            @endforeach
        </div>
        ...
        <div id="distribution" class="item pt-5 pb-5">
            <h2 class="text-lg font-medium">Distribución Logística histórica</h2>
            <p class="text-sm mb-3">Las siguientes son las operaciones registradas desde {{$reportDetail['initial_date']}} hasta
                {{$reportDetail['final_date']}}</p>
                ...
                <div class="box row-start-1 row-end-4 col-span-10 text-gray-600">
                    <div class="flex-inline text-xs">
                        <div class="item w-auto">
                            ...
                            <div class="text-sm font-bold tracking-wide text-purple-400">{{\Carbon\Carbon::createFromFormat('Y-m-d H:i:s',$o->opeDate)->format('d/m/Y')}}</div>
                        ...
                        <div class="item w-auto h-auto mt-4">
                            <div class="flex-inline">
                                ...
                                <div class="item text-xs font-bold uppercase tracking-wide text-gray-400 ">Sin información adicional</div>
                                @else
                                <div class="item text-xs font-bold uppercase tracking-wide text-gray-600 ">Comentarios</div>
...
<script>
    ...
    weeks.forEach(w => {
        ...
        const dConns = w.querySelectorAll("#dayConns");
    ...
    /*Distribution chart*/
    var ctx2 = document.getElementById('distChart').getContext('2d');
